  ...
  return (
    ...
    >
      ...
      <div className="z-10 max-w-2xl w-full mx-auto">
        ...
        >
          <div className="inline-flex items-center gap-2 px-3 py-1.5 md:px-4 md:py-2 bg-white/5 border border-white/10 rounded-full text-indigo-400 text-[10px] md:text-xs font-bold tracking-widest uppercase backdrop-blur-sm mb-3 md:mb-6">
          ...
          <p className="text-gray-400 text-sm md:text-lg">可多选,也可以直接开始探索更多可能性</p>
        </motion.div>
        ...
        <motion.div
          ...
          className="bg-white/5 backdrop-blur-md rounded-[24px] md:rounded-[32px] p-4 md:p-8 border border-white/10 shadow-2xl"
        >
          <div className="grid grid-cols-3 gap-2 md:gap-4 mb-4 md:mb-8">
            {MAJOR_OPTIONS.map((option, index) => (
              <motion.button
                ...
                className={`p-3 md:p-6 rounded-[16px] md:rounded-[24px] border-2 transition-all ${
                  selected.includes(option.value)
                    ? 'border-indigo-500 bg-indigo-500/30 shadow-lg shadow-indigo-500/20'
                    : 'border-white/10 bg-white/5'
                }`}
              >
                <div className="text-2xl md:text-4xl mb-1 md:mb-2">{option.icon}</div>
                <div className="text-xs md:text-sm font-bold text-white">{option.label}</div>
              </motion.button>
            ))}
          </div>
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            onClick={handleNext}
            className="w-full py-3.5 md:py-5 rounded-[20px] md:rounded-[24px] font-bold text-base md:text-lg transition-all bg-indigo-600 text-white hover:bg-indigo-700 shadow-xl shadow-indigo-600/30"
          >
